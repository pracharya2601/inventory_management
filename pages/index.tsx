...
const Home: NextPage = (props: any) => {
  ...
  const [value, loading, error] = useDocument(
    firebase.firestore().doc('workplaces/61325b3296e4eb481c819011'),
    {
  ...
  return (
    <div className="dark bg-white">
      ...
      <main className="bg-red-100 p-2 dark:bg-gray-900 min-h-screen">
        <button onClick={() => signOut()} className="bg-red-500 p-2 rounded">Sign Out</button>
        <button onClick={() => addWorkplace()} className="bg-green-500 p-2 rounded">Add Data</button>

        <div className="bg-white flex flex-col gap-3">
          {workplaces && workplaces.map(({ id, name }: { id: string, name: string }) => (
            <button key={id} onClick={() => router.push(`/workplaces/${id}`)} className="bg-blue-500 my-2 p-2 rounded">{name}</button>
          ))}
        </div>
      </main>
    </div >
