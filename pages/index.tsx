            ...
            <div className="w-full mt-10 p-5 sm:p-10 gap-3">
                <div className=" rounded p-5 sm:p-10">
                    ...
                    {reasons.map(({ heading, description }) => (
                        <div className="bg-gray-700 rounded px-3 py-5 my-4" key={`${heading}-reason`}>
                            <div className="text-xl md:text-2xl mb-2 font-semibold">{heading}</div>
                            <div>{description}</div>
                        </div>
                    ))}
                    <Button
                        label={'Create Company Dashboard'}
                        onClick={() => {
                            dispatch(
                                action.toggleAction({
                                    id: 'createworkplace',
                ...
                <div className="rounded p-5 sm:p-10">
                    <div className="text-2xl md:text-3xl my-2">
                        Inventory Management makes easier for user who work on multiple workplaces. The only thing you
                        have to do is veryfy the workplace you have been added from and start managing the inventory.
                    </div>
                    <div className="bg-gray-700 rounded px-3 py-5 my-4">
                        <div className="text-xl md:text-2xl mb-2 font-semibold">Join The Company</div>
                        <div>
    ...
    if (session && session.user) {
        if (session.user.name === null || session.user.image === null) {
            return {
                redirect: {
                    permanent: false,
                    destination: '/auth/newUser?type=new',
                },
            };
        }
    }
