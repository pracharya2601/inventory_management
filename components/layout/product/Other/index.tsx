                ...
                >
                    <div className="mt-1 max-w-5xl mx-auto px-1">
                        {dataList.length > 0 ? (
                            dataList.map((item: OtherType, index: number) => (
                                <div
                                    ...
                                    className={`border rounded mt-3 ${
                                        item?.deliveryAttempt === 'failed' && 'border-red-500'
                                    }`}
                                >
                                    ...
                                    <div className="">
                                        {item?.items?.map((singleItem: ProcessProductInfo, index: number) => (
                                            <div
                                                key={`${singleItem.name}-${index}`}
                                                className={`${index > 0 && 'border-t border-gray-600'} p-2 flex`}
                                            >
                                                ...
                                                <div className="m-1 flex-1">
                                                    <div className="capitalize text-sm sm:text-md md:text-lg font-semibold">
                                                        {singleItem.name}{' '}
                                                    </div>
                                                    <div className="text-xs mt-px mb-2 text-blue-200">
                                                        Order Created By:{' '}
                                                        <span className="ml-2 underline">
                                                            {item.orderCreatedBy?.name}
                                                        </span>
                                                    </div>
                                                    <div className="text-sm sm:text-md mt-1 w-40">
                                                        Color:{' '}
                                                        <span className="px-2 border py-px rounded">
                                                            {singleItem.color}
                                                        </span>
                                                    </div>
                                                    <div className="text-sm sm:text-md mt-1 w-40">
                                                        Size:{' '}
                                                        <span className="ml-2 md:ml-3 px-2 py-px border rounded">
                                                            {singleItem.size}
                                                        </span>
                                                    </div>
                                                    <div className="text-xs mt-2 text-green-300">
                                                        Numbero of Item:{' '}
                                                        <span className="ml-2">{singleItem.numberOfItem}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                    ...
                                    {item?.deliveryAttempt && (
                                        <div className="border-t py-1 bg-yellow-500 text-gray-900  text-center px-1 rounded-b">
                                            <span className="block text-sm text-md  uppercase font-semibold">
                                                Delivery Attempt: Failed
                                            </span>
                                            <span className="block text-sm text-red-900">
                                                Do not Removed order this will removed automatically after delivery
                                                company returned order
                                            </span>
                                        </div>
                                    )}
                                    {!item?.deliveryAttempt && (
                                        <div className="flex px-2 border-t rounded-b items-center bg-gray-700">
                                            <div className="w-30 p-1 text-center">
                                                <span className="block text-sm text-yellow-500 uppercase font-semibold">
                                                    Payment
                                                </span>
                                                <span className="block text-xs text-yellow-400">
                                                    {item.paymentStatus.method}
                                                </span>
                                            </div>
                                            {item.productType === 'processing' && (
                                                <div className="w-20 p-1 text-center">
                                                    <CancelButton
                                                        ...
                                                        label={
                                                            <span className="cursor-pointer block text-sm text-red-500 hover:underline uppercase font-semibold">
                                                                cancel
                                                            </span>
                                                        }
                                                    />
                                            ...
                                            <div className="w-40 max-w-xs overflow-x-hidden md:flex-1 p-1 text-center md:text-left truncate">
                                                {item.productType === 'processing' && (
                                                    <ChangeAddressButton
                                                        secret={business?.secret}
                                                        dataId={item._id}
                                                        customerData={item?.customerAddress}
                                                        customerName={item.customerInfo?.name}
                                                        label={
                                                            <span className="cursor-pointer block text-sm hover:underline text-purple-500 uppercase font-semibold">
                                                                Change address
                                                            </span>
                                                        }
                                                    />
                                                )}
                                                {item.productType === 'shipped' ||
                                                    (item.productType === 'delivered' && (
                                                        <>
                                                            <span className="block text-sm uppercase font-semibold">
                                                                Delivery By
                                                            </span>
                            ...
                            <div className="h-96 flex justify-center items-center text-center px-4">
                                No Available data. Previous data moved to different catagory.
                            </div>
                        )}
                    </div>
                </ProductLayout>
