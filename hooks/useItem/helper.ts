...
export const calculateCount = (
    ...
    checkingArr: string[],
    checkType: 'size' | 'color',
    countType: 'size' | 'color',
    checkVal: string,
) => {
    const a = arr.map((itm) => (checkingArr.includes(itm[checkType]) && itm[countType] == checkVal ? itm.count : 0));
    return a.reduce((prev, next) => prev + next);
};

export const calculateSpecificCount = (arr: Skus[], color: string, size: string) => {
    const a = arr.map((itm) => (itm.color == color && size == itm.size ? itm.count : 0));
    return a.reduce((prev, next) => prev + next);
};
