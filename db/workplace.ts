...
export const getOneWorkPlace = async (db: Db, id: string, userType: StaffType, userId: string) => {
  ...
  try {
    const data = await db.collection('workplaces').find({ "_id": ObjectId(id) }, { [`${userType}`]: { $elemMatch: { userId: userId } } }).toArray();
...
export const createWorkPlace = async (db: Db, workplace: { workplaceName: string, workplaceCode: string }, user: { userId: string, fullName: string, email: string }) => {
